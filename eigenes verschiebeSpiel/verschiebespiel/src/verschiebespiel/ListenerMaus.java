...
public class ListenerMaus extends MouseAdapter implements ItemListener, ActionListener {
	...
	public void actionPerformed(ActionEvent event) {
		if (event.getActionCommand() == bild.getActionCommand()){
			...
			fchooser.setDialogTitle("Bitte wähle ein Bild für das freie Feld");
			returnVal = fchooser.showOpenDialog(sf);
			
			if (returnVal == JFileChooser.APPROVE_OPTION){
				pfad = fchooser.getSelectedFile().getAbsolutePath();
				icon = new ImageIcon(pfad);
			}
			sf.setIconLeeresLabel(icon);
		}
